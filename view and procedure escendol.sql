			...
			open cur1;
				looping:loop 
					...
					ELSE
						...
						END IF;
						INSERT INTO growth_vlr VALUES(nama_csm, tanggal, vlr, vlr_sebelumnya, (concat(round(( abs(vlr-vlr_sebelumnya)/vlr_sebelumnya * 100 ),2),'%')), keterangan);
					END IF;
				end loop looping; 
			close cur1; 
			SELECT v.nama_csm, v.tanggal, v.vlr, v.vlr_sebelumnya, v.persentase, v.keterangan FROM (SELECT MAX(x.vlr) AS max FROM growth_vlr x WHERE MONTH(x.tanggal) = MONTH(keyword) AND YEAR(x.tanggal) = YEAR(keyword)) m, growth_vlr v WHERE MONTH(v.tanggal) = MONTH(keyword) AND YEAR(v.tanggal) = YEAR(keyword) AND v.vlr = m.max;
		END IF;
	END$$
...
SELECT CELLID, MIN(HSDPA_USER_W36)
